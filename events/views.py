...
# Q objects: Django Q objects allow more specific database queries to be
# processed. They are useful when multiple search items need to be combined
# using OR (|), AND (&) or NOT (~)
...
def event_list(request):
    ...
    # Get the search query from the "q" query-string parameter submitted by the
    # search form. If no query is provided, return an empty string so the
    # search input remains blank and its placeholder text is displayed.
    search_query = request.GET.get("q", "")

    if search_query:
        events = events.filter(
            # icontains =  A lookup function that is not case-sensitive.
            # Searches for the search_query in the title, venue, location,
            # genre, and lineup fields of the Event model, using OR ("|") to
            # combine the conditions. This allows the user to find events that
            # contain any of this information in their fields.
    ...
    # Create a context dictionary to pass the events to the template as Django
    # needs a context dictionary to render the template with the events data.
    # Which the template can iterate through to display the events.
    context = {
        "events": events,
...
def event_detail(request, slug):
    ...
    # The is_favourited variable is calculated in the view because the view is
    # responsible for preparing data for the template. This keeps the template
    # focused on displaying the data rather than performing database queries.
    # The template can then simply check whether is_favourited is True or False
    # to display the correct favourite button.
...
def add_comment(request, slug):
    ...
    # Check if the request method is POST, which indicates that the user has
    # submitted the comment form.
    if request.method == "POST":
        comment_form = CommentForm(request.POST)
        # Check if the form is valid (all required fields are filled out
        # correctly).
        if comment_form.is_valid():
            comment = comment_form.save(commit=False)

            # Link the comment to the selected event. Prevents the user from
            # choosing which event to comment on, as the comment is
            # automatically linked to the event detail page they are on.
            comment.event = event

            # Link the comment to the currently logged-in user. Prevents the
            # user from choosing which user to comment as, as the comment is
    ...
    # Redirect the user back to the event detail page after submitting the
    # comment.
...
def toggle_favourite(request, slug):
    ...
    # Check if the request method is POST, confirming that the user has
    # clicked the favourite/unfavourite button.
    if request.method == "POST":
        ...
        # If the user has not favourited the event, add them to the
        # favourited_by list.
        else:
            event.favourited_by.add(request.user)
            messages.success(request, "Event added to your favourites.")

    # Redirect the user back to the event detail page after toggling the
    # favourite status.
    return redirect("event_detail", slug=event.slug)


# Decorator "@login_required" to ensure that only logged-in users can access
# the event_create view.
@login_required
def event_create(request):
    """
    View for creating a new event. Only accessible to logged-in users.
    """
    # Check if the request method is POST, which shows that the user has
    # submitted the form.
    if request.method == "POST":
        # Create an instance of the EventForm with the submitted data and
        # files.
        form = EventForm(request.POST, request.FILES)
        # Check if the form is valid (all required fields are filled out
        # correctly).
        if form.is_valid():
            # Save the form but don't commit to the database yet, so we can set
            # the creator field
    ...
    # Else -If the request method is not POST, create a new instance of the
    # EventForm to display an empty form to the user.
    else:
        form = EventForm()
    # Create a context dictionary to pass the form to the template for
    # rendering
    context = {
        "form": form,
    }
...
def dj_profile_create(request):
    ...
    # Prevent a user from creating more than one DJ profile. If a DJ profile
    # already exists for the logged-in user, they are redirected to the
    # homepage with a warning message.
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            # Save the form without committing it to the database yet, if it
            # was committed now, the owner field would be empty and cause an
            # error.
            profile = form.save(commit=False)

            # Automatically assign the logged-in user as the owner, so they
            # cannot create a DJ profile
            # for someone else.
            ...
            # Display a success message to the user after successfully creating
            # their DJ profile.
            messages.success(
                request,
            ...
            # Redirect the user to the homepage after successfully creating
            # their DJ profile.
            return redirect("home")

    # Else - If the request method is not POST, create a new instance of the
    # DJProfileForm to display an empty form to the user.
    else:
        form = DJProfileForm()
    ...
    # Render the dj_profile_form.html template with the context containing the
    # form
    return render(
        request,
...
def event_edit(request, slug):
    ...
    """
    # Asks the database for the event with the given slug. If no event is
    # found, a 404 error page will be returned.
    event = get_object_or_404(Event, slug=slug)

    # This prevents one user from editing another user's event. If the
    # logged-in user is not the creator of the event, they are redirected to
    # the event detail page instead of being allowed to edit it. This adds a
    # layer of security to ensure that only the creator of the event can make
    # changes to it.
    if event.creator != request.user:
        return redirect("event_detail", slug=event.slug)

    # Check if the request method is POST, which shows the user has submitted
    # the form.
    if request.method == "POST":
        ...
        form = EventForm(
            ...
            # instance=event tells Django to update the existing Event instance
            # with the new data from the form instead of creating a new one.
            instance=event,
            )
        ...
        # correctly).
        if form.is_valid():
            # I used event = form.save() to save the form because the existing
            # event instance already has a creator, so it does not need to be
            # set again. The form.save() method will update the existing event
            # instance with the new data from the form.
            event = form.save()

            # Redirect user to the event detail page after successful edit
            return redirect("event_detail", slug=event.slug)

    # Else - If the request method is not POST, create an instance of the
    # EventForm with the existing event data to display the form pre-filled
    # with the current event details.
    else:
...
def event_delete(request, slug):
    ...
    # Prevent logged-in users from deleting events created by someone else.
    # This has been recycled from the event_edit view to ensure that only the
    # creator of the event can delete it.
    if event.creator != request.user:
        return redirect("event_detail", slug=event.slug)

    # Only delete the event after the confirmation form is submitted.
    # This has also been recycled from the event_edit view to ensure that the
    # event is only deleted after the user confirms.
    if request.method == "POST":
        event.delete()
...
def dj_profile_list(request):
    ...
    """
    # Retrieve all DJ profiles. Filter not required as DJ profiles don't have
    ...
    # Create a context dictionary to pass the DJ profiles to the template as
    # Django needs a context dictionary to render the template with the DJ
    # profiles data. Which the template can iterate through to display the DJ
    # profiles.
    context = {
...
def dj_profile_detail(request, slug):
    ...
    # Create a context dictionary to pass the DJ profile to the template as
    # Django needs a context dictionary to render the template with the DJ
    # profile data. Which the template can use to display the DJ profile
    # details.
    context = {
        "dj_profile": dj_profile,
    }

    # Render the dj_profile_detail.html template with the context containing
    # the DJ profile
    return render(
        request,
...
# Decorator "@login_required" to ensure that only logged-in users can access
# the dj_profile_edit view.
@login_required
def dj_profile_edit(request, slug):
    """
    Allow a DJ profile owner to edit their own profile.
    """
    # Retrieve the DJ profile with the matching slug. If no profile is found,
    # return a 404 page.
    dj_profile = get_object_or_404(DJProfile, slug=slug)

    # Prevent users from editing another user's DJ profile. If the logged-in
    # user is not the owner of the DJ profile, they are redirected to the
    # DJ profile detail page instead of being allowed to edit it.
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Redirect the user to the DJ profile detail page after
            # successfully editing their DJ profile.
            return redirect(
                "dj_profile_detail",
                slug=dj_profile.slug
            )
    ...
    # Else - If the request method is not POST, create an instance of the
    # DJProfileForm with the existing DJ profile data to display the form
    # pre-filled with the current DJ profile details.
    else:
        # Reuse the DJProfileForm with the existing DJ profile instance
        # to display the user's current profile details in the form.
        form = DJProfileForm(instance=dj_profile)

    # Create a context dictionary to pass the form and DJ profile to the
...
def dj_profile_delete(request, slug):
    ...
    # they are redirected to the DJ profile detail page instead of being
    ...
    # If the request method is POST, delete the DJ profile and redirect the
    # user to the DJ profile list page with a success message. So the DJ
    # profile is only deleted after the user confirms the deletion.
    if request.method == "POST":
        dj_profile.delete()
    ...
    # If the request method is GET, display the confirmation page to the user
    # before deleting the DJ profile.
    context = {
        "dj_profile": dj_profile,
    }

    # Render the dj_profile_confirm_delete.html template with the context
    # containing the DJ profile
    return render(
        request,
...
def dashboard(request):
    ...
    # Retrieve the DJ profile owned by the logged-in user, if one exists.
    # .first() returns None if the user has not created a profile yet.
    dj_profile = DJProfile.objects.filter(
        owner=request.user
    ).first()

    # Retrieve all events created by the logged-in user.
    # Draft events are included because this is the user's private dashboard.
